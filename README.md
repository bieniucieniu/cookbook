...
apps/
  ...
  web      React (Vite) on :3000, proxies /api → :8080
packages/
...
Server needs `WORKOS_API_KEY`, `WORKOS_CLIENT_ID`, `WORKOS_COOKIE_PASSWORD` (32+ chars). Optional `WORKOS_COOKIE_SECURE=true` in production.

### Run

- Server: `devenv --profile server up` or `gradle :apps:server:run`
